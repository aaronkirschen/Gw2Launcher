...
namespace Gw2Launcher
{
    static class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            if (args.Length > 0)
            {
                #region -restore [path] [pid] [-preview]
                ...
                if (args[0] == "-restore")
                {
                    ...
                    if (args.Length > 3 && args[3] == "-preview")
                    {
                        Settings.ReadOnly = true;

                        var import = new Tools.Backup.Backup.Importer(args[1]);
                        var ri = import.ReadInformation();
                        ...
                        try
                        {
                            var path = Path.Combine(DataPath.AppDataAccountDataTemp, "settings.dat");
                            ri.Extract(ri.Settings, path);

                            Settings.Load(path);

                            using (var mf = MemoryMappedFile.OpenExisting(Messaging.MappedMessage.BASE_ID + "Restore:" + pid))
                            {
                                using (var writer = new BinaryWriter(mf.CreateViewStream()))
                                {
                                    writer.Write(pid);
                                    ...
                                    foreach (var a in Util.Accounts.GetAccounts())
                                    {
                                        ...
                                        writer.Write(a.Name);
                                        writer.Write(a.LastUsedUtc.ToBinary());
                                    }
                    ...
                    Application.Run(new UI.Backup.formBackupRestoreImport(args[1], pid));
                ...
                #region -backup [path] [-d -xlocal -xgfx]
                ...
                if (args[0] == "-backup")
                {
                    ...
                    var path = args[1];
                    if (!path.EndsWith(".bkp", StringComparison.OrdinalIgnoreCase))
                        path += ".bkp";

                    var o = new Tools.Backup.Backup.BackupOptions()
                    {
                        Format = Tools.Backup.Backup.BackupFormat.File,
                        IncludeGfxSettings = true,
                        IncludeLocalDat = true,
                    };
                    ...
                    for (var i = 2; i < args.Length; i++)
                    {
                        switch (args[i].ToLower())
                        {
                            ...
                            case "-xlocaldat":
                            case "-xlocal.dat":
                                o.IncludeLocalDat = false;
                                break;
                            case "-xgfx":
                            case "-xgfxsettings":
                            case "-xgfxsettings.xml":
                                o.IncludeGfxSettings = false;
                                break;
                            case "-d":
                                o.Format = Tools.Backup.Backup.BackupFormat.Directory;
                                break;
                        }
                    }
                    ...
                    try
                    {
                        backup.Export(CancellationToken.None);
                    }
                    catch (Exception e)
                    {
                        return e.HResult;
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        if (MessageBox.Show("Unable to start due to an abandoned mutex", "Mutex was abandoned", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) != DialogResult.Retry)
                            return e.HResult;
                        try
                        {
                            Util.ProcessUtil.KillMutex(MUTEX_NAME, Win32Handles.MatchMode.EndsWith, true);
                        }
                        catch { }
                    }
                    catch(Exception e)
                    {
                        MessageBox.Show("An error occured while trying to create a mutex\n\n" + e.Message, "Unable to create mutex", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return e.HResult;
                    }
                }
        ...
        public static void Uninstall(bool restore)
        {
            Client.FileManager.Uninstall();
